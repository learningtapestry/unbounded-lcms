...
class CurriculumExporterTest < ActiveSupport::TestCase
  def test_all_grades
    exporter = CurriculumExporter.new
    ...
    assert_equal ['ID Our Database', 'Title', 'Subtitle', 'Description', 'URL', 'Grades', 'Standards', 'Resource Types', 'Subjects'], sheet.row(1)
  ...
  def test_grade1
    ...
    exporter = CurriculumExporter.new([grade1.id])
    data = exporter.export
